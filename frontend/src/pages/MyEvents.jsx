import { useState } from "react";import { Link } from "react-router-dom";import { CalendarX, Ticket } from "lucide-react";
import Navbar from "../components/Navbar";
import TicketCard from "../components/TicketCard";
import { mockEvents } from "../data/mockEvents";
import { myUpcomingEventIds, myPastEventIds } from "../data/mockRegistrations";

function MyEvents() {
  const [activeTab, setActiveTab] = useState("upcoming");

  const idsToShow = activeTab === "upcoming" ? myUpcomingEventIds : myPastEventIds;
  const myEvents = mockEvents.filter((event) => idsToShow.includes(event.id));
    ...
    <div className="min-h-screen bg-orange-50/30 flex flex-col">
      ...
      <div className="flex-1 max-w-4xl mx-auto w-full px-8 pt-10 pb-16">
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div>
            <h1 className="text-3xl md:text-4xl font-bold text-gray-900">Mes Événements</h1>
          ...
          <div className="flex bg-gray-100 rounded-lg p-1">
            <button
              ...
              className={`text-sm font-medium px-4 py-2 rounded-md transition ${
                activeTab === "upcoming" ? "bg-white text-primary shadow-sm" : "text-gray-600"
              }`}
            >
              À venir
            </button>
            <button
              ...
              className={`text-sm font-medium px-4 py-2 rounded-md transition ${
                activeTab === "past" ? "bg-white text-primary shadow-sm" : "text-gray-600"
              }`}
            >
              Événements passés
            </button>
          ...
          <div className="flex flex-col items-center text-center mt-16">
            <div className="relative w-48 h-48 flex items-center justify-center">
              <div className="absolute w-40 h-40 bg-white rounded-2xl shadow-sm rotate-6 flex items-center justify-center">
                <CalendarX size={56} strokeWidth={1.2} className="text-orange-200" />
              </div>
            ...
            <h2 className="text-xl font-bold text-gray-900 mt-8">
              {activeTab === "upcoming" ? "Aucun événement trouvé" : "Aucun événement passé"}
            </h2>
